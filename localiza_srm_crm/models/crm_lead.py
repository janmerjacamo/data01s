...
class CrmLead(models.Model):
    ...
    localiza_owner = fields.Selection(
        LOCALIZA_OWNER_SELECTION,
        string='Propietario de la Oportunidad',
        tracking=True,
        help='Responsable comercial Localiza. Sustituye el campo Studio de propietario.'
    )
    ...
    localiza_weighted_revenue = fields.Monetary(
        ...
        help='Sustituye el Monto Esperado de Studio. Se calcula según probabilidad de etapa.'
    )
    localiza_stage_history_ids = fields.One2many(
        'localiza.crm.stage.history', 'lead_id', string='Historial de fases'
    )
    localiza_stage_history_count = fields.Integer(
        string='Historial de fases', compute='_compute_localiza_counts'
    )
    localiza_calendar_event_ids = fields.One2many(
        'calendar.event', 'localiza_opportunity_id', string='Eventos/Reuniones Localiza'
    )
    localiza_calendar_event_count = fields.Integer(
        string='Eventos/Reuniones', compute='_compute_localiza_counts'
    )
    ...
    def _compute_localiza_counts(self):
        ...
        for lead in self:
            lead.localiza_stage_history_count = History.search_count([('lead_id', '=', lead.id)])
            lead.localiza_calendar_event_count = Event.search_count([('localiza_opportunity_id', '=', lead.id)])
    ...
    def create(self, vals_list):
        for vals in vals_list:
            if vals.get('stage_id') and 'probability' not in vals:
                ...
                vals['probability'] = self._localiza_stage_probability(stage)
    ...
    def _localiza_create_stage_history(self, old_stage, new_stage):
        ...
        if last and last.change_date:
            delta = fields.Datetime.now() - last.change_date
